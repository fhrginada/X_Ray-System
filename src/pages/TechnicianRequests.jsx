...
const TechnicianRequests = ({ onSelectRequest, requests: externalRequests, onChangeStatus }) => {
  const requests = externalRequests?.length ? externalRequests : mockRequests;
  ...
  const paginated = filteredRequests.slice((page - 1) * 6, page * 6);
  const totalPages = Math.max(1, Math.ceil(filteredRequests.length / 6));
            ...
            {paginated.length > 0 ? (
              paginated.map((request) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 align-top">
                    <Badge variant={request.status === 'completed' ? 'success' : request.status === 'pending' ? 'warning' : 'info'} className="status-badge text-[11px]">
                      {request.status}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 align-top text-right">
                    {request.status === 'pending' ? (
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="outline" onClick={() => onChangeStatus?.(request.id, 'accepted')}>
                          Accept
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => onChangeStatus?.(request.id, 'rejected')}>
                          Reject
                        </Button>
                      </div>
                    ) : request.status === 'accepted' || request.status === 'in-progress' ? (
                      <Button size="sm" className="action-btn upload-btn" onClick={() => onSelectRequest(request)}>
                        Upload Images
                      </Button>
                    ) : (
                      <Button
