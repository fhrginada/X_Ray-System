...
const DoctorXrayView = () => {
  ...
  const images = selectedPatient
    ? mockImages.filter((img) => img.patientId === selectedPatient.patientId)
    : [];

  const handleSelectPatient = (request) => {
  ...
  return (
    <div className="doctor-view mx-auto max-w-6xl px-1 py-3 sm:px-2 md:px-0 md:py-4">
        ...
        <div className="flex flex-wrap gap-2">
          {mockRequests.map((req) => (
            <button
              key={req.id}
              type="button"
              onClick={() => handleSelectPatient(req)}
              className={`rounded-2xl border px-4 py-2 text-left text-sm transition-all ${
                selectedPatient?.patientId === req.patientId
                  ? 'border-sky-400/80 bg-sky-500/10 text-sky-300 shadow-soft-card'
                  : 'border-slate-700/70 bg-slate-900/80 text-slate-300 hover:border-sky-400/50 hover:bg-slate-900'
              }`}
            >
              <span className="font-medium">{req.patientName}</span>
              <span className="ml-2 text-xs text-slate-500">#{req.patientId}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {!selectedPatient ? (
        <div className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-slate-800/80 bg-slate-950/70 text-center text-slate-400">
          <div className="mb-4 text-5xl">👤</div>
          <h3 className="mb-1 text-sm font-semibold text-slate-100">Select a patient above</h3>
          <p className="text-xs text-slate-500">Choose a patient to view their X-ray images</p>
        </div>
      ) : (
        <>
      {/* Top Bar */}
      ...
      <div className="viewer-container grid gap-4 rounded-2xl border border-slate-800/80 bg-slate-950/70 p-3 shadow-soft-elevated md:grid-cols-[260px,minmax(0,1fr)] md:p-4">
        ...
        <div className="thumbnails-sidebar flex flex-col overflow-y-auto rounded-2xl border border-slate-800/80 bg-slate-950/80 px-3 py-4">
          ...
          <div className="thumbnails-list flex flex-col gap-2">
            {images.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-4">No images available</p>
            ) : images.map((image) => (
              <button
                key={image.id}
                type="button"
