...
const PatientBooking = ({ onBookingComplete }) => {
  ...
  const handleToggle = (field) => {
    setBookingData((prev) => ({ ...prev, [field]: !prev[field] }));
  };
            ...
            {step === 1 && (
              <>
                ...
                <form className="booking-form grid max-w-2xl gap-4">
                  ...
                  {/* Clinic Referral */}
                  <div className="form-group rounded-2xl border border-slate-700/60 bg-slate-900/50 px-4 py-3">
                    <label className="flex cursor-pointer items-center gap-3">
                      <input
                        type="checkbox"
                        checked={bookingData.isReferred}
                        onChange={() => handleToggle('isReferred')}
                        className="h-4 w-4 rounded border-slate-600 bg-slate-900 accent-sky-400"
                      />
                      <span className="text-sm font-medium text-slate-200">
                        Referred from a clinic
                      </span>
                    </label>
                    {bookingData.isReferred && (
                      <div className="mt-3">
                        <label className="mb-1.5 block text-xs font-medium uppercase tracking-[0.18em] text-slate-400">
                          Clinic Name
                        </label>
                        <Input
                          type="text"
                          name="clinicName"
                          value={bookingData.clinicName}
                          onChange={handleInputChange}
                          placeholder="Enter referring clinic name"
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="form-group rounded-2xl border border-slate-700/60 bg-slate-900/50 px-4 py-3">
                    <label className="flex cursor-pointer items-center gap-3">
                      <input
                        ...
                        onChange={() => handleToggle('hasInsurance')}
                        className="h-4 w-4 rounded border-slate-600 bg-slate-900 accent-sky-400"
                      />
                      <span className="text-sm font-medium text-slate-200">
                        I have insurance
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            value={bookingData.insurancePolicyNumber}
                            onChange={handleInputChange}
              ...
              <>
                ...
                <div className="confirmation-card mb-6 rounded-2xl border border-slate-700/70 bg-slate-900/70 px-5 py-4 shadow-soft-card md:px-6 md:py-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {bookingData.isReferred && (
                      <div className="summary-row flex items-start gap-3 border-t border-slate-800/80 pt-2 text-sm">
                        <span className="label w-24 shrink-0 text-xs font-medium uppercase tracking-[0.18em] text-slate-400">
                          Referral
                        </span>
                        <span className="value text-slate-50">
                          {bookingData.clinicName || 'Clinic not specified'}
                        </span>
                      </div>
                    )}
                    ...
                    {bookingData.hasInsurance && (
                      <div className="summary-row flex items-start gap-3 border-t border-slate-800/80 pt-2 text-sm">
                        ...
                        <span className="value text-slate-50">
                          {bookingData.insuranceProvider || 'Provider not specified'}
                          {bookingData.insurancePolicyNumber && (
